...
class Address extends Db {
    private $tablePhysical = 'addresses';
    private $tableLogical = '住所';
     ...
     * @return DynamicProperty $results 
     */
    public function insertInitial($user_id = null) {

        $results = $this->initResults();
        $this->transaction();
    
        try {
            $sql = $this->insertInitialSql();
            $sth = $this->pdo->prepare($sql);
            $params = array(':user_id' => $user_id);
            // SQL実行結果                   : $sth->execute($params)
            // SQL実行件数                   : $sth->rowCount()
    ...
    private function insertInitialSql() {
        $sql = "";
